...
def get_args_parser():
    ...
    parser.add_argument('--BCELoss_ratio', default=0.3, type=float,
                        help='Weight ratio for Binary Cross Entropy Loss (0.0-1.0), should sum to 1 with DiceLoss_ratio')
    parser.add_argument('--DiceLoss_ratio', default=0.7, type=float,
    ...
    parser.add_argument('--dataset_path', default="../data/MID",
                        help='Root directory path for dataset')
    parser.add_argument('--batch_size_train', type=int, default=2,
                        help='Number of samples per training batch (RTX 4060 optimized: 4-6)')
    parser.add_argument('--batch_size_test', type=int, default=3,
                        help='Number of samples per test batch (can be larger than train)')
    parser.add_argument('--lr_scheduler', type=str, default='CosLR',
                        help='Learning rate scheduler type [PolyLR|StepLR|CosLR] - CosLR for faster convergence')
    parser.add_argument('--lr', default=5e-4, type=float,
                        help='Initial learning rate (increased for faster training)')
    parser.add_argument('--min_lr', default=1e-5, type=float,
                        help='Minimum learning rate for PolyLR')
    parser.add_argument('--weight_decay', default=0.009, type=float,
                        help='Weight decay coefficient (reduced for faster convergence)')
    parser.add_argument('--epochs', default=50, type=int,
                        help='Total number of training epochs (reduced with better params)')
    ...
    parser.add_argument('--num_threads', default=4, type=int,
                        help='Number of subprocesses for data loading (RTX 4060 optimized)')
    ...
    parser.add_argument('--mixed_precision', action='store_true', default=True,
                        help='Enable mixed precision training for RTX 4060 (faster + less memory)')
    parser.add_argument('--gradient_accumulation_steps', type=int, default=1,
                        help='Gradient accumulation steps (increase if batch size too small)')
    ...
    parser.add_argument('--non_blocking', action='store_true', default=True,
                        help='Non-blocking data transfer to GPU')
    return parser
